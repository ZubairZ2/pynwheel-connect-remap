namespace :db do
  ...
  task :create_communities => :environment do
    ...
    Community.where(name: "ellis", data_provider: "entrata", company_id: 1).first_or_create # PSI
    Community.where(name: "rent cafe", data_provider: "yardirentcafe", company_id: 1).first_or_create # YardiRentCafe
    Community.where(name: "integra creek", data_provider: "realpage", company_id: 1).first_or_create # RealPageSvc
