...
Change appointment #{scheduler_widget_change_tour_time_url(schedual_tour)}?datetime=#{get_date_time_combined(schedual_tour.tour_date, schedual_tour.tour_time).to_s}
...
#{community.one_hour_email_text}"
					...
					DelayedSchedulerMailerJob.perform_async("Your tour starts soon!", content, tu.email,nil,nil,nil)
