  ...
  if @community.show_map
    ...
    if @community.is_sitemap
      if scheduled_tour_stops.present?
        ...
        unoccupied = tour.tour_stops.where(stop_type: "unit").map{|x| x.id if (u = Unit.find x.stop_id) and !u.available and !u.modal_unit }.compact
        stops_arr = @community.mdu ? tour.tour_stops.where(display_stop: true).where.not(id: unoccupied).order(:sort) : tour.tour_stops.where.not(display_stop: false,stop_type: "unit").order(:sort)
      end
        ...
        if stop_lock_provider == "Latch" and @community.latch.present? and stop.latch_locks.present?
          ...
          if zrv.present?
            ...
            if zrv_guest.present?
              ...
              json.guest_pin 'Your tour has started. The door will unlock when your mobile device is within range. Enjoy your tour!'
              json.latch_link ''
              json.unit_dwelo_lock_id ''
              json.igloohome_lock_id ''
              json.igloohome_guest_bluetooth_key ''
      ...
      if @community.enable_locks and @tour_user.tour_type != "virtual_tour"
        ...
        if stop_lock_provider == "EdgeState"
          ...
          if zrv.present?
            ...
            if zrv_guest.present?
              is_zerv_lock_present = true
              list_of_zerv_lock_ids = @tour_user.get_list_of_zerv_lock_ids(tour, @community, stop, new_stops_arr, counter)
