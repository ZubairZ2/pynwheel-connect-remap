...
json.units_available @community.community_tour&.tour_setting&.enable_tour_customization ? (@community.is_sitemap ? (@community.units.available_units.count > 0) : (@community.units.available_units.where.not(floor: [nil], building: ["", nil, "N/A"]).count > 0) ) : false
json.amenities_available @community.community_tour&.tour_setting&.enable_tour_customization ? (@community.is_sitemap ? (@community.amenities.plotted_amenities.count > 0) : (@community.amenities.plotted_amenities.where.not(floor: [nil], building: ["", nil, "N/A"]).count > 0) ) :false
