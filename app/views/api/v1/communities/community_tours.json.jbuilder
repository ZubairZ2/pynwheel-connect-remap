json.tours @tours do |tour|
  ...
  json.tour_stop tour.tour_stops.order(:sort) do |stop|
    ...
    if stop.stop_type == "unit"
      ...
      stop_dat = {"floorplan" => Floorplan.find_by(provider_floorplan_id: unit.floorplan_id).name,"effective_rent" => unit.effective_rent,"available_date" => unit.available_date,"lease_pricing" => unit.lease_pricing,"availability" => unit.availability,"stop_description" => unit.stop_description, "availability_url"=> unit.availability_url.present? ? unit.availability_url :  Floorplan.find_by(provider_floorplan_id: unit.floorplan_id).availability_url}
