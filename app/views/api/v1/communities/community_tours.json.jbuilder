json.tours @tours do |tour|
  ...
  json.tour_stop tour.tour_stops.order(:sort) do |stop|
    ...
    if stop.stop_type == "unit"
      ...
      stop_dat = {"marketing_name" => unit.marketing_name,"floorplan" => Floorplan.find_by(provider_floorplan_id: unit.floorplan_id).name,"effective_rent" => unit.effective_rent,"available_date" => unit.available_date,"availability" => unit.availability,"stop_description" => unit.stop_description}
