...
json.apartments do
  ...
  json.units available_units_and_sold_units.each do |unit|
    if floorplans.any?{|f| f.provider_floorplan_id == unit.floorplan_id}
      ...
      json.secondary_image unit.secondary_image.present? ? (Rails.env.development? ? local_assets_base_url+unit.secondary_image.url : unit.standard_image_url.url) : (floorplan.present? && floorplan.secondary_image.present? ? (Rails.env.development? ? local_assets_base_url+floorplan.secondary_image : floorplan.secondary_image.url) : nil)
