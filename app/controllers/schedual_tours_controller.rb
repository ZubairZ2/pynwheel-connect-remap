class SchedualToursController < ApplicationController
  ...
  private
    ...
    def send_email_and_other_notifications schedual_tour
      ...
      email_content = "<div style='vertical-align:middle; text-align:center'><img style='width: 100px; max-height: 100px;' src='#{community.logo.present? ? community.logo.url : ''}' data-title='#{community.name}' /></div><br/>Thank you for scheduling your self-guided tour! We look forward to having you at the property(<b>#{community.name if community.present?}</b>) on  <b>#{schedual_tour.tour_date.strftime("%A, %b %-d %Y")}</b> at <b>#{ Time.parse(schedual_tour.tour_time.to_s).strftime("%I:%M %P")}</b>. When you go to the property, you will need <br/> <ul><li>A photo ID</li> <li>Your mobile device with the Pynwheel Self Tour app installed.</li></ul>Please download the Pynwheel self-guided tour app: before you arrive: <br><a href=#{app_link} target='_blank'>Download Pynwheel Self Tour From App Store </a><br><a href=#{android_link} target='_blank'>Download Pynwheel Self Tour From Google Play</a> <br> #{community.email_text}"
      ...
      sms_content = "Thank you for scheduling your self-guided tour! We look forward to having you at #{community.name if community.present?} on #{schedual_tour.tour_date.strftime("%A, %b %-d %Y")} and #{ Time.parse(schedual_tour.tour_time.to_s).strftime("%I:%M %P")}. When you go to the property, you will need
-A photo ID
