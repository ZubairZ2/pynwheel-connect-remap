...
class Users::SessionsController < Devise::SessionsController
  ...
  def destroy
    begin
      puts "in sesion controller"
      # LoggedInUser.where(browser_id: cookies[:browser_id]).destroy_all          # delete current user record
      LoggedInUser.where(user_id: current_user.id).destroy_all          # delete current user record
      -----------201 `  1`
      ...
      req_communities = LoggedInUser.where.not(browser_id: cookies[:browser_id]).present? ? [] : (chat_enabled_communities_for_current_user - chat_enabled_communities_for_logged_in_users)
