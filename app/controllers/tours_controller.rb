  ...
  def flag_id_mismatch
    ...
    if params[:tour_user_id].present?
      ...
      if tour_user.id_selfie_mismatch
        ...
        DelayedSchedulerMailerJob.perform_async("ID / Selfie Matching (Manual)", email_content, 'usman.khalid@intagleo.co.uk',community)
        if community_email.present?
          emails = community_email.gsub(" ","").split(',')
          emails.each do |email|
            DelayedSchedulerMailerJob.perform_async("User #{name} is marked Mismatched ", email_content, email,community)
          end
          # DelayedSchedulerMailerJob.perform_async("User #{name} is marked Mismatched ", email_content, community_email)
        else
          DelayedSchedulerMailerJob.perform_async("User #{name} is marked Mismatched ", email_content, 'jennifer@pynwheel.com',community) unless params[:local_testing].present?
        end
      end
    else
      status = 404
