class ToursController < ApplicationController
  def index
    ...
    @community.tour.tour_stops.order(:sort).each {|x| x.stop_type.classify.constantize.find_by_id(x.stop_id).paths.each{|z| @existing_path_points << z.path_points.reorder('id ASC') if (z.path_points.present? &&  !floorplate_units.include?(z.map_path_from_id)  ) } if (x.present? && @all_stops.include?(x.stop_id)) }
