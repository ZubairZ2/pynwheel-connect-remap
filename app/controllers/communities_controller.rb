class CommunitiesController < ApplicationController
  ...
  def alert_message
    ...
    elsif params[:community][:map_logo].present?
      '<div class="alert alert-success">Map Logo updated successfully.</div>'
  ...
  @community.enable_pynwheel_pricing_calculator = params[:enable_pynwheel_pricing_calculator].present? ? params[:enable_pynwheel_pricing_calculator] : false
  ...
  def community_params
    params.require(:community).permit(:use_company_level_data_settings, :show_amenity_name,:property_manager_name,:property_manager_phone,:property_manager_email, :enable_three_d_maps,:default_satellite_view,:web_map_type,:enable_amenity_legend,:community_logo,:pynwheel_access,:name,:creator_id,:default_community_id, :region_id,:billing_rate_touch,:billing_rate_for_both, :lincoln_billing_rate,:dwelo_billing_rate , :billing_rate_selftour, :billing_rate_maps,:address,:number_of_units,:city,:state,:zip,:phone,:email,:description, :manual_lat_long,:latitude,:longitude,:company_id,:logo, :map_logo, :secondary_logo,:self_tour_logo, :email_logo, :restrict_access,:scheduler_widget,:pynwheel_touch,
      :auto_wayfinding, :data_provider,:theme_name,:code,:is_sitemap,:menu_button_shade,:enable_locks,:locked,:website,:equal_housing_opportunity_logo,:handicap_accessible_logo,:powered_by_btn,:tour_setup_visible, :chat_control, :self_tour, :pynwheel_launch_access, :show_map, :mdu, :touchscreen_app,:apply_now_pynwheel_touch_and_go,:apply_now_pynwheel_touch,:apply_now_self_tour, :show_gesture_icons,:billing_type,:billing_rate,:date_installed,:billing_month,:is_vertical_app,:enable_svg_mode,:is_beans_svg,:enable_sdk_map,:enable_sdk_map_cache,:enable_pricing_calculator,:pricing_calculator_embed_code,:enable_pynwheel_pricing_calculator,
      :credential_attributes=>[:unit_name_key, :currency, :yardi_rent_cafe_api_url, :entrata_available_units_only, :entrata_show_unit_spaces, :entrata_use_space_configuration,:id,:url, :apartmentlist_url, :entrata_url,:rentmanager_username,:rentmanager_password,:rentmanager_property_id, :rentmanager_base_url, :username,:password, :perq_property_id, :is_perq_allowed, :property_id, :app_folio_property_id, :app_folio_property_group_id, :app_folio_property_scope, :app_folio_database_id,:pmc_id,:server_name,:database,:platform,:interface_entity,:site_id,:c_code,
