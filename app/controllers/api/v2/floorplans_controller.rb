class Api::V2::FloorplansController < Api::V2::ApiApplicationController
  ...
  after_action :update_floorplan_form_status, except: [:index, :save_floorplans_form]
  ...
  def save_floorplans_form
    params[:floorplans]&.each do |floorplan_param|
      ...
      floorplan.update_attributes(name: floorplan_param[:name], virtual_tour_url: floorplan_param[:virtual_tour_url], description: floorplan_param[:description]) if floorplan.present?
      update_floorplan_amenities_info(floorplan, floorplan_param[:floorplan_amenities])
    end
  ...
  private
    ...
    def update_floorplan_amenities_info floorplan, floorplan_amenities
      floorplan_amenities&.each do |floorplan_amenity_param|
        if floorplan.present?
          ...
          floorplan_amenity.update_attributes(name: amenity_gallery_param[:name], description: amenity_gallery_param[:description])
    ...
    def update_floorplan_form_status
