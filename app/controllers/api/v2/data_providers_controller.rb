class Api::V2::DataProvidersController < Api::V2::ApiApplicationController
  ...
  def update_data_provider
    ...
    @community.update_columns(data_provider: data_provider)
  end
  ...
  def create_company_level_credential(company)
    ...
    if provider == 'yardi'
      company.credential.update_columns(yardi_username:  params["credential"]["username"], yardi_password: params["credential"]["password"])
    end
    company_credentials
  end
  ...
  def credential_params
    ...
    params.require(:credential).permit(:id,:url,:entrata_url,:username,:password, :perq_property_id, :is_perq_allowed,
      ...
      :zaremba_property_id,:zaremba_username,:zaremba_password,:new_requested_data_provider, :rentmanager_username, :rentmanager_password, :rentmanager_property_id, :rentmanager_base_url)
  end
  ...
  def crm_credential_params
    ...
    params.require(:crm_credential).permit(:crm_provider, :entrata_domain, :entrata_username, :entrata_password, :entrata_property_id,
      ...
      :salesforce_client_id, :salesforce_secret_id, :salesforce_property_id, :knock_community_id, :knock_company_id, :salesforce_grant_type, :funnel_community_id, :funnel_api_key)
  end
