    ...
    floorplans_params.each do |floorplan|
      if floorplan["id"].present? && @community.floorplans.find_by_id(floorplan["id"]).present?
        ...
        if find_floorplan.update(name: floorplan["name"], image: floorplan["image"], secondary_image: floorplan["secondary_image"])
          ...
          @floorplan << { success: true, data: find_floorplan.as_json }
        else
          @floorplan << { success: false, data: find_floorplan.errors.full_messages.join(',') }
        end
      else
        new_floorplan = @community.floorplans.new(name: floorplan["name"], image: floorplan["image"], secondary_image: floorplan["secondary_image"])
        if new_floorplan.save
          ...
          @floorplan << { success: true, data: new_floorplan.as_json }
        else
          @floorplan << { success: false, data: new_floorplan.errors.full_messages.join(',') }
        end
      end
    end
    if @floorplan.any?
