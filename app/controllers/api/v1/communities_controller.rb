    ...
    class CommunitiesController < BaseController
      ...
      def portico_list_communities
        ...
        if @tour_user.present?
          @communities = Community.select(:id,:name,:email,:phone,:company_id,:locked,:latitude,:longitude,:address,:logo,:state,:city).includes(:company, :allowed_emails).self_tour_enabled_only.where(allowed_emails: {email: [@tour_user.email]})
        else
