class Api::V1::SalesforceWebhooksController < ActionController::Base
  ...
  def salesforce_tour_webhook
    begin
      ...
      # schedual_tour = MaxDateScheduledTourService.new(@tour_user, @community, true).get_scheduled_tour
      if @scheduled_tour.present?
        new_tour.update(stops_list: @scheduled_tour.stops_list)
      else
      ...
      schedual_tour.update_attributes(community_id: community_id, tour_user_id: @tour_user.id ,user_time_zone: timezone, tour_date: tour_date, tour_time: params[:tourTime], tour_type: params[:tourType], stops_list: @stops_list, created_by: "salesforce")
      # if @scheduled_tour.present?
