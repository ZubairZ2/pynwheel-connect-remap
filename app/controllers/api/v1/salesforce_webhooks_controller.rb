    ...
    class SalesforceWebhooksController < BaseController
      ...
      def create_or_update_tour_user
        if !@tour_user.present?
          ...
          @tour_user.update_columns(first_name: params[:neighborFirstName], last_name: params[:neighborLastName], name: params[:neighborFirstName] + " " + params[:neighborLastName], email: params[:neighborEmail].downcase, phone_number: (make_phone_number).present? ? make_phone_number : @tour_user.phone_number) 
        end
