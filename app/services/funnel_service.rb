class FunnelService < BaseService
  ...
  def is_appointment_created response
    if ( response && response["prospect"] && response["appointment"] && response["prospect"]["id"] && response["appointment"]["id"] ).present?
      @scheduled_tour.update_columns(funnel_prospect_id: response["prospect"]["id"], funnel_appointment_id: response["appointment"]["id"])
    end
  end

  def is_appointment_cancelled
    @scheduled_tour.update_columns(funnel_prospect_id: nil, funnel_appointment_id: nil)
  end
