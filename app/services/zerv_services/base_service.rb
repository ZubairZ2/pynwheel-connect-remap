module ZervServices
  class BaseService
    ...
    def current_community_zerv_locks(locks)
      ...
      locks["listGetDevices"] = locks["listGetDevices"].map{|lock| lock if lock["locationName"].present? && ( (lock["locationName"].downcase.parameterize.gsub("-", "").gsub("_", "") == location_name) || (community.address.present? && lock["locationName"] == community.address ) ) }.compact
