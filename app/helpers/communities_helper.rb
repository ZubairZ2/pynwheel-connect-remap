module CommunitiesHelper
  ...
  def write_feedback_report(workbook)
    ...
    worksheet.write(0, 4, "Rating",format)
    worksheet.write(0, 5, "Comment",format)
    worksheet.write(0, 6, "Cancelled",format)
    worksheet.write(0, 7, "Feedback Created Date",format)
    
    Feedback.all.each do |tf|
      if tf.present?
        ...
        worksheet.write(row, 4, tf.rating,format1)
        worksheet.write(row, 5, tf.comment,format1)
        worksheet.write(row, 6, tf.is_cancelled,format1)
