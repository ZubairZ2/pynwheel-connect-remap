class Hallway < ApplicationRecord
  ...
  def delete_hallway_point(hallways)
    ...
    if linked_hallways.count == 0 && linked_by_hallways.count == 0 # for solo hallway point like me
      ...
      linked_hallways.each {|hallway| hallway.next_points = (hallway.next_points - [self.id]).distinct; hallway.save }
      self.destroy
      hallway = linked_hallways.first; hallway.selected = true; hallway.save
    elsif linked_hallways.count == 0  # For leaf point
      linked_by_hallways.each {|hallway| hallway.next_points = (hallway.next_points - [self.id]).distinct; hallway.save }
      self.destroy
      ...
      if linked_hallways.count == 1 && linked_by_hallways.count == 1
        ...
        prev_point.next_points = (prev_point.next_points - [self.id]).distinct
        prev_point.next_points = (prev_point.next_points + [next_point.id]).distinct
        prev_point.save
        self.destroy
        ...
        linked_by_hallways.each do |hallway|
          hallway.next_points = (hallway.next_points - [self.id]).distinct
          hallway.next_points = (hallway.next_points + [linked_hallways.first.id]).distinct
          hallway.save
        end
        self.destroy
        hallway = linked_hallways.last; hallway.selected = true; hallway.save
        ...
        hallway.next_points = (hallway.next_points - [self.id]).distinct
        hallway.next_points = (hallway.next_points + linked_hallways.pluck(:id)).distinct
        hallway.save
        self.destroy
        hallway = linked_hallways.last; hallway.selected = true; hallway.save
      end
