...
#         logged_in_user.logged_in_count = 0 if logged_in_user.logged_in_count > 0 and Community.joins(:users).where(users: {id: current_user.id}).where(community_users: {chat_enable: true}).pluck(:is_chat_login).distinct == [false]     # if somehow is_chat_login for user chat_enabled_comuunities is false but somehow logged_in count becomes > 0 (rescued)
...
#         # subscribers = connections.map{|connection| connection[:current_user].id}.distinct.count
...
#                     chat_enabled_communities_for_current_user = CommunityUser.where(user_id: current_user.id, chat_enable: true).pluck(:community_id).distinct   # current user's chat enabled communities
...
#                         offline_communties = CommunityUser.joins("INNER JOIN logged_in_users ON logged_in_users.user_id = community_users.user_id").where(logged_in_users: {logged_in_count: 0}).where(community_users: {chat_enable: true, community_id: req_communities}).pluck(:community_id).distinct
